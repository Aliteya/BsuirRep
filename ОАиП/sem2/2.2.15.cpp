...
void filesort(char& path, FILE* my_file, int& flag) {
	...
	flag = 1;
	int start = 0, end = n - 1;
	cout << "Выберите сортировку: 1 - линейный, 2(не работает) - быстрая сортировка " << endl;
	cin >> choise;
	switch (choise) {
...
void basicsort(equipment* repair2, int n) {
	for (int i = 0; i < n - 1; i++) {
		for (int j = i; j < n; j++) {
			if (strcmp(repair2[i].data, repair2[j].data) == true)
				swap(repair2[i].data, repair2[j].data);
		}
	}
}
